...
<html>
<head>
<title>OpenArchaeo Explorateur | Explorer</title>
<meta charset="utf-8">
...
PREFIX oa: <http://www.openarchaeo.fr/explorateur/onto#>

SELECT  ?site ?nom
WHERE  {
  ?site a oa:Site .
  OPTIONAL { ?site skos:prefLabel ?nom . }
}'}</textarea>
					<ul class="fa-ul">
	...
	<script type="text/javascript">
	 ...
	 function changeView(val){
		 ...
		 $.ajax({
	         url : 'expand',
	         type : 'POST', 
	         data : 'query=' + encodeURIComponent(sparql) + '&sources=${sources}' , 
	        	...
	        	CodeMirror.fromTextArea(document.getElementById("expandedQuery"), {
	   			...
	   			var query= response.expandQuery;
	   			 $.ajax({
	   		         ...
	   		         success: function(response) {
	   		        	$('#sparqlResultLabel').html('RESULTAT SPARQL :');
	   		        	$("#yasr").html('');
	   		        	var yasr = YASR(document.getElementById("yasr"));
	   		            yasr.setResponse(response);
	   		          },
	   		       	  error: function(response) {
	   		        	$('#sparqlResultLabel').html('ERREUR SPARQL !');
	   		          }
	   		      });
	          }
	      });
